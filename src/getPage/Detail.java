	...
	public static void GetDetail(Map<String, String> urlList) throws  IndexOutOfBoundsException{
		...
		for(Map.Entry<String, String> url : urlList.entrySet()) {
			try {
				...
				String price = "";
				if(priceText.matches("^[0-9]*$")) {//値段が数字になっているか
					price = priceText;
				}
				...
				String productText = "";//最終の商品のテキスト
				String appendText = "";//一文一文の商品テキスト

				//商品テキスト1をラインごとに取得し、<br>と共に足し合わせる
				//テキストを一文一文確認し、NGワードが入っていた場合、その文章を吹き飛ばす
...
//				System.out.println(asin);
	...
	private static String toStr(LocalDateTime localDateTime, String format) {
